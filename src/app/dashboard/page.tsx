...
export default function DashboardPage() {
  ...
  const [view, setView] = useState<"posts" | "gallery">("posts")
  ...
  const handleDeleteGallery = (_id: string) => {
    setConfirmAction(() => async () => {
      try {
        setGalleryLoading(true);
        const res = await fetch("/api/gallery", {
      ...
      <div className="flex flex-row justify-between items-center">
        ...
        >
          <ToggleGroupItem
            ...
            className={cn(
              "px-4 py-2",
              view === "posts" ? "bg-yellow-900 text-yellow-200" : "text-gray-700"
            )}
          >
            Posts
          </ToggleGroupItem>
          <ToggleGroupItem
            ...
            className={cn(
              "px-4 py-2",
              view === "gallery" ? "bg-yellow-900 text-yellow-200" : "text-gray-700"
            )}
          >
            Gallery
          </ToggleGroupItem>
        ...
        <div className="flex flex-col bg-yellow-50 m-1">
          ...
          <div className="mt-6 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {gallery.map((img) => (
              <div
                key={img._id}
                className="relative bg-gradient-to-b from-yellow-100 to-yellow-200 p-2 rounded-lg shadow"
              >
                <Button
                  onClick={() => handleDeleteGallery(img._id)}
                  className="absolute top-2 z-10 right-2 bg-red-900 text-white text-xs p-1 rounded hover:bg-red-800"
                >
                  <X className="h-2 w-2"/>
                </Button>
